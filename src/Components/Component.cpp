...
void Component::DrawColliderOutline(void* drawListRaw, float windowWidth, float windowHeight, float viewScale, uint32_t color) const
{
	if (!hasPhysicBody || !shape || !drawListRaw) return;
	ImDrawList* drawList = static_cast<ImDrawList*>(drawListRaw);
	cpBody* body = cpShapeGetBody(shape);
	auto ToScreen = [windowWidth, windowHeight, viewScale](const cpVect& p) {
		return ImVec2(
			windowWidth * 0.5f + static_cast<float>(p.x) * viewScale,
			windowHeight * 0.5f - static_cast<float>(p.y) * viewScale
		);
	};
...
void Component::DrawColliderOutlineRecursive(void* drawList, float windowWidth, float windowHeight, float viewScale, uint32_t color) const
{
	DrawColliderOutline(drawList, windowWidth, windowHeight, viewScale, color);
	for (const Component* child : children)
	{
		if (child)
			child->DrawColliderOutlineRecursive(drawList, windowWidth, windowHeight, viewScale, color);
	}
}
